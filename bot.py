...
CATEGORIES = {
    ...
    "✈️ Путешествия": [
        "Виза / разрешение", "Жилье", "Аренда машины", "Бензин", "Кафе/рестики", "Продукты/Перекусы", "Билеты", "Связь", "Другое",
    ],
    "🎁 Другое": [
...
async def quick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "quick_save":
        ...
        await query.edit_message_text("Хорошо, переключаю на пошаговый ввод с уже заполненными данными...")
        parsed = context.user_data.get("quick_parsed", {})
        context.user_data.clear()
